...
from pyspark.sql.functions import col, count, count_if, from_json, explode,from_unixtime,current_timestamp
import dlt
...
def earthquake_data():
    df = (
        ...
        .load(volume_path)
        .withColumn("_long_timestamp",current_timestamp())
    )
    df = df.withColumn("parsed_data", from_json(col("features"), schema))
    df = df.select(explode(col("parsed_data")).alias("features"),"_long_timestamp")
    df = df.select(
        "features.properties.*",
        "features.id",
